...
pub fn _try_grow(stack_size: usize, callback: &mut dyn FnMut()) -> Result<(), StackError> {
    ...
    unsafe {
        ...
        if data.parent_fiber.is_null() {
            return Err(StackError::last_os_error(
        ...
        if fiber.is_null() {
            let error = StackError::last_os_error("unable to allocate fiber");
            if !was_fiber {
