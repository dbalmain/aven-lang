...
impl StackRestoreGuard {
    pub fn new(requested_size: usize) -> Result<StackRestoreGuard, StackError> {
        ...
        unsafe {
            ...
            if new_stack == libc::MAP_FAILED {
                return Err(StackError::last_os_error("mmap failed to allocate stack"));
            }
            ...
            if result == -1 {
                let error = StackError::last_os_error("mprotect/mmap failed");
                drop(guard);
                return Err(error);
            }
            Ok(guard)
...
mod tests {
    ...
    fn test_stack_area() {
        for stack_size_kb in 1..64 {
            ...
            let stack = StackRestoreGuard::new(size).unwrap();
