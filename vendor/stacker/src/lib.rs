...
/// An operating-system failure while allocating a stack segment.
#[derive(Debug)]
...
impl StackError {
    ...
    fn invalid_input(message: &'static str) -> Self {
        Self {
            operation: "invalid stack allocation",
...
pub fn grow<R, F: FnOnce() -> R>(stack_size: usize, callback: F) -> R {
    match try_grow(stack_size, callback) {
        Ok(value) => value,
...
/// Always creates a new stack for the passed closure to run on.
///
/// Unlike [`grow`], allocation failure is returned before `callback` runs.
/// Panics raised by `callback` continue to unwind normally.
pub fn try_grow<R, F: FnOnce() -> R>(stack_size: usize, callback: F) -> Result<R, StackError> {
    ...
    _try_grow(stack_size, dyn_callback)?;
    Ok(ret.expect("the stack callback runs before stack growth returns"))
}
...
psm_stack_manipulation! {
    ...
    no {
        ...
        fn _try_grow(stack_size: usize, callback: &mut dyn FnMut()) -> Result<(), StackError> {
            let _ = stack_size;
            callback();
            Ok(())
        }
        #[cfg(all(windows, not(miri)))]
        use backends::windows::_try_grow;
    }
}
