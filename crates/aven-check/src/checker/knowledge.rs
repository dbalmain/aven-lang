...
/// How a value came to be known does not appear here.
///
/// It used to: a `Provenance` said whether the author wrote `comptime(...)` or
/// the checker simply found an ordinary call it could evaluate, and an
/// unasked-for proof was allowed to answer less. Both kinds are equally *true*,
/// and the settled decision is that a demand is answered by the value rather
/// than by who asked for it, so the distinction had nothing left to decide.
/// Reintroduce it only for something it can actually decide --- a budget, a
/// diagnostic --- never as a second assignability rule beside this one.
#[derive(Debug, Clone)]
...
impl<'a> Checker<'a> {
    ...
    /// satisfied specialization --- and an ordinary call whose inputs all
    /// happened to be known. They record the same kind of thing, because a
    /// value is the value whoever asked for it.
    ///
    ...
    /// What a proof can do is answer a demand --- see
    /// `demand_is_discharged_by_knowledge`.
